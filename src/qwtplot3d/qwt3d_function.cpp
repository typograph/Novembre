...
bool Function::create()
{
  ...
  /* allocate some space for the mesh */
   double* data         = new double [umesh_p*vmesh_p] ;
  ...
  for (i = 0; i < umesh_p; ++i)
  {
    for (j = 0; j < vmesh_p; ++j)
    {
      data[i*vmesh_p + j] = operator()(minu_p + i*dx, minv_p + j*dy);

      if (data[i*vmesh_p + j] > range_p.maxVertex.z)
        data[i*vmesh_p + j] = range_p.maxVertex.z;
      else if (data[i*vmesh_p + j] < range_p.minVertex.z)
        data[i*vmesh_p + j] = range_p.minVertex.z;
    }
  }
