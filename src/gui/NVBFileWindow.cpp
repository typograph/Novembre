...
NVBFileWindow::NVBFileWindow(NVBWorkingArea * area, const QModelIndex & index, NVBFile * model, NVB::ViewType stateMode)
	...
	NVBDataSource * page = hardlinkDataSource(index.data(PageRole).value<NVBDataSource*>());
...
void NVBFileWindow::addSource(const QModelIndex & index) {
	addSource(hardlinkDataSource(index.data(PageRole).value<NVBDataSource*>()));
	}
...
void NVBFileWindow::dragMoveEvent(QDragMoveEvent * event) {
	...
	if (event->answerRect().intersects(pageListView->geometry()))
#if QT_VERSION >= 0x040300
...
void NVBFileWindow::showPageOperationsMenu(const QModelIndex & index, const QPoint & pos) {
	...
	if (a->text() == "Open in new window") {
		...
		openInNewWindow(index);
//		openInNewWindow(hardlinkDataSource(index.data(PageRole).value<NVBDataSource*>()),NVBVizUnion(),NVB::TwoDView);
		}
...
//		openInNewWindow(hardlinkDataSource(index.data(PageRole).value<NVBDataSource*>()),NVBVizUnion(),NVB::GraphView);
		}
...
//		openInNewWindow(hardlinkDataSource(index.data(PageRole).value<NVBDataSource*>()),NVBVizUnion(),NVB::ThreeDView);
		}
