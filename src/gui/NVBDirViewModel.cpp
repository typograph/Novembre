 ...
 * This class is the model for the DirView. In the `normal' mode it is
 * a depth-2 tree with files as level-1 nodes and datasets as level-2 nodes.
 * The file has no decoration, and the display string is a '\n'-separated string 
 * of all the parameters displayed in the file list.
 * The decoration role on a page returns the same picture of a page NVBFileModel returns,
 * unless the picture generation is overwridden. At the moment, only one generator
 * is available, the NVBSpecOverlayIconProvider. TODO - General generator interface.
 * 
 * In the NVBDirViewModel::SingleImage mode, the model is flattened to a list. Level-1
 * nodes are still files, but now their decoration is an image generated by the 
 * NVBFile2ImageConverter. 
 * 
 */
...
NVBDirViewModel::NVBDirViewModel(NVBFileFactory * factory, NVBDirModel * model, QObject * parent)
	...
	, mode(Normal)
	, firstConverter(new NVBShowFirstPageConverter()){
...
QVariant NVBDirViewModel::data( const QModelIndex & index, int role ) const {
	...
	if (index.internalId() == 0) { // This is a file
		if (mode == SingleImage) {
		...
		if (role == Qt::DisplayRole /*StatusTipRole*/) {
			// Get column model of the DirView, or whatever it was that owned the files
			const QAbstractItemModel * m = indexes.at(index.row()).model();

			// I guess this was unloaded
			if (!m) return QVariant(); // FIXME - persistent indexes have to be updated

			// OK, the indexes that we have are just file parameters. We need the directory index
			QModelIndex pi = indexes.at(index.row()).parent();
			// And the row
			int row = indexes.at(index.row()).row();
			// And the number of user columns
			int c = m->columnCount(pi);
			
			QStringList texts;

			for(int i = 0; i < c; i++) {
				// This is the text written in that column
				texts << m->index(row, i, pi).data(Qt::DisplayRole).toString();
				// Or not
				if (texts.last().isEmpty()) texts.removeLast();
				}
			// Compact and be happy
			return texts.join("\n");
			}
		// Anything else (e,g, StatusTipRole) can be taken from up there.
		return indexes.at(index.row()).data(role);
		}
	else { // This is a page in a file
		if (files.at(index.internalId() - 1)) { // File is loaded
			if (mode == SpectroscopyOverlay && role == Qt::DecorationRole) // Loaded altered icon
...
void NVBDirViewModel::setOverlayActive(bool active)
{
	if (active != overlayActive) {
		overlayActive = active;
		if (mode != SingleImage)
			setMode(active ? SpectroscopyOverlay : Normal);
		}
}

void NVBDirViewModel::setSingleImageModeActive(bool active)
{
	if (active ^ mode == SingleImage)
		setMode(active ? SingleImage : overlayActive ? SpectroscopyOverlay : Normal);
}
