//
// C++ Interface: NVBGeneralDelegate
//
// Description: 
//
//
...
public:
  ...
  virtual QwtText trackerText(const QwtDoublePoint & pos) const {
    if (const QwtPlot * p = plot()) {
      switch(rubberBand()) {
        case HLineRubberBand: return p->axisScaleDraw(yAxis())->label(pos.y());
        case VLineRubberBand: return p->axisScaleDraw(xAxis())->label(pos.x());
        default:
