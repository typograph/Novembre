...
NVBFileStruct * WinSPMFileGenerator::loadFile(QString filename)
{
//   NVBDataSource * ds;
  QFile * file = 0;
...
// --------- read first directory
  ...
  quint32 strip_offset, strip_rows, strip_bytes;
...
*/
  ...
  s->pages.append(new WinSPMTopoPage(file,header,strip_offset,color_map_tag.data));

  if (header.cits_offset) {
    file->seek(header.cits_offset);
    s->pages.append(new WinSPMCITSPage(file,header));
//     s->pages.append(WinSPMCITSPage::slices(file,header));
  ...
  if (header.sps_offset) {
    ...
    for (int i=0; i < header.n_of_sps; i++) {
      ...
      s->pages.append(new WinSPMSpecPage(file,header,specheader,next_sps_offset));
      }
    }
  else {
    QFileInfo info(*file);
    ...
    QStringList l;
    l << QString("%1*.spc").arg(info.baseName());
//    NVBOutputVPMsg("Looking for spec files",QString("%1?.spc").arg(info.baseName()));
    QFileInfoList spcs = dir.entryInfoList(l,QDir::Files,QDir::Name);
    
    if (!spcs.isEmpty()) {
    
      WinSPMSpecPage * specpage = new WinSPMSpecPage(header);
    
      foreach(QFileInfo i, spcs) {
        QFile specfile(i.absoluteFilePath());
...
NVBFileInfo * WinSPMFileGenerator::loadFileInfo(QString filename)
{
...
WinSPMTopoPage::WinSPMTopoPage( QFile * const file, const TWinSPM::Header & _header, quint32 data_offset, quint32 color_offset ):header(_header)
{
  pagename = "Topography";
...
void WinSPMFileGenerator::getWinSPMHeader( TWinSPM::Header & header, QFile * const file )
{
...
WinSPMSpecPage::WinSPMSpecPage(QFile * const file, const TWinSPM::Header & topo_header, const TWinSPM::Header & _header, quint32 data_offset):theader(topo_header)
{
  pagename = "Spectroscopy";
...
void WinSPMSpecPage::addCurve( QFile * const file, const TWinSPM::Header & header, quint32 data_offset )
{
...
WinSPMCITSPage::WinSPMCITSPage( QFile * const file, const TWinSPM::Header & _header ):header(_header) {
  pagename = "Spectroscopy";
...
void WinSPMCITSPage::getCITSHeader( TWinSPM::CITS_Header & header, QFile * const file )
{
  file->read((char*)&(header.winspm_version),2);
...
void WinSPMCITSPage::getCITSImageHeader( TWinSPM::CITS_ImageHeader & header, QFile * const file )
{
  file->read((char*)&(header.adc_min),2);
  file->read((char*)&(header.adc_max),2);
...
QList< NVBDataSource * > WinSPMCITSPage::slices( QFile * const file, const TWinSPM::Header & header )
{
